...
@Command
public class SphereCommand implements JfbCommand {
    ...
    private int execute(CommandSourceStack source, int size, boolean useDiameter, boolean hollow)
        ...
        state.setContext(new SphereContext(size, useDiameter, hollow));
        CommandUtil.pushPreviewSnapshots(player, state);
        CommandUtil.sendMessage(source, CommandUtil.translate("command.jfb.sphere.success",
